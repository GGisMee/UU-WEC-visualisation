...
class DrivetrainSpecs(TypedDict):
    ...
    downtime: float
    drivetrain_cost_modifier: float
    opex_modifier: float
...
DRIVETRAIN_SPECS: dict[tuple[Gearbox, Generator], DrivetrainSpecs] = {
    (Gearbox.HIGH_SPEED, Generator.DFIG):          {"drivetrain_efficiency": 0.94, "downtime": 0.05, "drivetrain_cost_modifier": 1.00, "opex_modifier": 1.00, "nacelle_mass_modifier": 1.00, "description": "A high gear ratio steps up the low rotor speed (ω) to high velocities. This mechanical amplification allows for a smaller generator size and lower weight. However, the high speed increases friction losses and heat within the gear teeth."},
    (Gearbox.MEDIUM_SPEED, Generator.SYNCHRONOUS): {"drivetrain_efficiency": 0.95, "downtime": 0.03, "drivetrain_cost_modifier": 1.08, "opex_modifier": 0.80, "nacelle_mass_modifier": 1.05, "description": "A moderate gear ratio reduces mechanical stress and kinetic energy inside the transmission. Paired with a synchronous generator, it achieves good magnetic flux linkage at lower shaft speeds. This setup balances the torque-speed trade-off without requiring an oversized stator diameter."},
    (Gearbox.NONE, Generator.SYNCHRONOUS):         {"drivetrain_efficiency": 0.96, "downtime": 0.02, "drivetrain_cost_modifier": 1.25, "opex_modifier": 0.50, "nacelle_mass_modifier": 1.50, "description": "The generator rotor is locked directly to the low-speed turbine shaft. To induce grid voltage at this low speed, the synchronous generator needs a massive diameter with many magnetic pole pairs. This eliminates gearbox wear completely but increases structural weight in the nacelle."},
    (Gearbox.HIGH_SPEED, Generator.ASYNCHRONOUS):   {"drivetrain_efficiency": 0.88, "downtime": 0.06, "drivetrain_cost_modifier": 0.90, "opex_modifier": 1.15, "nacelle_mass_modifier": 1.05, "description": "A maximum gear ratio provides the high speed needed to create magnetic slip in the induction machine. While this high velocity keeps the generator volume small, the extra gear stages introduce higher friction. The setup shows how mechanical speed can compensate for a basic generator topology."},
    (Gearbox.MEDIUM_SPEED, Generator.ASYNCHRONOUS): {"drivetrain_efficiency": 0.89, "downtime": 0.05, "drivetrain_cost_modifier": 1.00, "opex_modifier": 1.10, "nacelle_mass_modifier": 1.00, "description": "Operating this induction generator at a moderate speed reduces high-frequency drivetrain vibrations. However, lowering the input shaft velocity compresses the available magnetic slip range. Consider how this shifting operating point affects the torque-speed curve and power conversion."},
    
    # Orealistiska/Straffade kombinationer
    (Gearbox.NONE, Generator.DFIG):                {"drivetrain_efficiency": 0.70, "downtime": 0.20, "drivetrain_cost_modifier": 3.00, "opex_modifier": 2.00, "nacelle_mass_modifier": 2.50, "description":"Without a gearbox, the DFIG rotor spins at the very low velocity of the main shaft. DFIG machines rely on high relative speeds between stator and rotor fields to induce voltage. Consider how the generator size must scale to achieve rated power when input RPM is this low."},
    (Gearbox.MEDIUM_SPEED, Generator.DFIG):        {"drivetrain_efficiency": 0.90, "downtime": 0.06, "drivetrain_cost_modifier": 1.15, "opex_modifier": 1.10, "nacelle_mass_modifier": 1.10, "description":"This setup lowers the generator input speed by using a smaller gear ratio. Since a DFIG is optimized for a specific, higher velocity window, a medium input speed shifts the system away from its design point. Look at how a lower mechanical step-up impacts the excitation efficiency."},
    (Gearbox.NONE, Generator.ASYNCHRONOUS):         {"drivetrain_efficiency": 0.50, "downtime": 0.30, "drivetrain_cost_modifier": 4.00, "opex_modifier": 3.00, "nacelle_mass_modifier": 4.00, "description":"Connecting an asynchronous machine directly to the slow turbine rotor removes the speed multiplier. Asynchronous induction relies heavily on high slip speeds to create a strong magnetic field. To couple electromagnetically at low RPM, the design requires an extreme number of pole pairs."},
    (Gearbox.HIGH_SPEED, Generator.SYNCHRONOUS):    {"drivetrain_efficiency": 0.85, "downtime": 0.15, "drivetrain_cost_modifier": 2.00, "opex_modifier": 1.50, "nacelle_mass_modifier": 1.50, "description":"Using a high-speed gearbox with a synchronous generator is generally inefficient. Synchronous generators can easily be designed with multiple poles for lower speeds, making the high-speed gearbox an unnecessary source of mechanical failure and friction losses."},
}
@dataclass
class WindTurbine:
    ...
    @property
    def drivetrain_cost_modifier(self) -> float:
        """Cost modifier for CAPEX (drivetrain)."""
        return self.drivetrain_specs["drivetrain_cost_modifier"]

...
class EnergyCalculations:
    ...
    def calculate(self) -> CalculationResults:
        ...
        rated_power = (
            ...
            * rated_speed**3
            * swept_area
            * self.input_data.capture_efficiency
            * self.input_data.efficiency_drivetrain
            / 1000
        )
        ...
        alternatives = [
            ...
            * swept_area
            * self.input_data.capture_efficiency
            * self.input_data.efficiency_drivetrain,  # zone 2
            rated_power
        ...
        storm_load = (
            1 / 2 * 1.2 * 1.5 * solidity / 100 * swept_area * 60**2 / 1000
        )  #  @60 [kN], max at 60 kN. load under storm

        wall_thickness_operation = (
            aerodynamical_load
            * self.input_data.height
            / (np.pi * (self.input_data.height / 40) ** 2 * 160)
            * 2
        )
        ...
        self.output_data = CalculationResults(
            ...
            cut_in,
            generated_energy,
            average_power=generated_energy
            / 8.76,  # [KWh], 8.76 from hours in year / 1000
            full_load_hours=generated_energy
            / rated_power
            * 1000,  # [h], energy produced relative to maximum capacity
            aerodynamical_load=aerodynamical_load,
            solidity=solidity,  # [%] antal blad * bladens bredd / rotorns radie

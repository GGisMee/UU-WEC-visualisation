...
class AnalyticsPanel(ctk.CTkFrame):
    def __init__(self, parent, on_simulate_click):
        ...
        for key, label, init_val in audit_defs:
            ...
            ctk.CTkLabel(row, text=label, font=("Arial", 11), text_color=FusionTheme.TEXT_MUTED.value).pack(side="left")
            val_lbl = ctk.CTkLabel(row, text=init_val, font=("Arial", 11, "bold"), text_color=FusionTheme.TEXT_MAIN.value)
        ...
        ctk.CTkLabel(guidelines_box, text="DESIGN GUIDELINES", font=("Arial", 10, "bold"), text_color=FusionTheme.ACCENT.value).pack(anchor="w", padx=10, pady=(8, 2))
        ...
        ctk.CTkLabel(guidelines_box, text=desc_guidelines, font=("Arial", 10), text_color=FusionTheme.TEXT_MUTED.value, justify="left").pack(anchor="w", padx=10, pady=(0, 8))
        ...
        ctk.CTkLabel(self.ledger_scroll, text="CAPEX COST ALLOCATION BREAKDOWN", font=("Arial", 10, "bold"), text_color=FusionTheme.TEXT_MUTED.value).pack(anchor="w", padx=5, pady=(5, 2))
        ...
        for item in ledger_defs:
            ...
            lbl_color = FusionTheme.TEXT_MAIN.value if is_bold else FusionTheme.TEXT_MUTED.value
            
            ctk.CTkLabel(row, text=label, font=("Arial", 11, lbl_weight), text_color=lbl_color).pack(side="left")
            val_lbl = ctk.CTkLabel(row, text=init_val, font=("Arial", 11, "bold"), text_color=FusionTheme.INFO.value if is_bold else FusionTheme.TEXT_MAIN.value)
            val_lbl.pack(side="right")
        ...
        self.warning_banner = ctk.CTkFrame(self, fg_color="#332B12", height=32, corner_radius=0)
        self.lbl_warning = ctk.CTkLabel(
            self.warning_banner, 
            text="⚠️ Inputs changed. Click 'Run Simulation' to recalculate results.",
            font=("Arial", 11, "bold"),
            text_color=FusionTheme.ALERT.value
        )
        self.lbl_warning.pack(pady=4)

        # Loading Overlay (covers entire panel during simulation run)
        self.loading_overlay = ctk.CTkFrame(self, fg_color=FusionTheme.BG_SURFACE.value, corner_radius=0)
    ...
    def display_results(self, result: SimulationResult):
        ...
        thick_color = FusionTheme.DANGER.value if result.mean_wall_thickness > 150.0 else FusionTheme.TEXT_MAIN.value
        ...
        sm_color = FusionTheme.SUCCESS.value if safety_margin >= 1.5 else (FusionTheme.ALERT.value if safety_margin >= 1.0 else FusionTheme.DANGER.value)
        ...
        # Extract scalar value if it's root_scalar result
        if hasattr(irr_val, 'root'):
            irr_text = f"{irr_val.root * 100:.1f} %"
        self.ledger_rows["irr"].configure(text=irr_text)

        # Profit Margin (multiply by 100 to get %)
        margin_val = result.margin * 100.0
        margin_color = FusionTheme.SUCCESS.value if margin_val >= 0 else FusionTheme.DANGER.value
    ...
    def clear_charts(self):
        ...
        self.charts_canvas.create_text(
            ...
            fill=FusionTheme.TEXT_MUTED.value[1], 
            font=("Arial", 12), 
            justify="center"
        )
    ...
    def draw_performance_curves(self):
        ...
        self.charts_canvas.create_rectangle(x1_offset, y_offset, x1_offset + plot_w, y_offset + plot_h, fill="#0F172A", outline=FusionTheme.BORDER.value[1])
        self.charts_canvas.create_text(x1_offset + plot_w // 2, y_offset - 10, text="Weibull Wind Speed Curve", fill=FusionTheme.INFO.value[1], font=("Arial", 9, "bold"))
        ...
        if len(points) > 1:
            self.charts_canvas.create_line(points, fill=FusionTheme.INFO.value[1], width=2)

        # Shaded operational limits
        cut_in_x = int((res.cut_in_speed / max_v) * plot_w)
        cut_out_x = int((res.cut_out_speed / max_v) * plot_w)
        self.charts_canvas.create_line(x1_offset + cut_in_x, y_offset, x1_offset + cut_in_x, y_offset + plot_h, fill=FusionTheme.SUCCESS.value[1], dash=(4, 4))
        self.charts_canvas.create_text(x1_offset + cut_in_x + 12, y_offset + 15, text="Cut-in", fill=FusionTheme.SUCCESS.value[1], font=("Arial", 7), angle=90)

        self.charts_canvas.create_line(x1_offset + cut_out_x, y_offset, x1_offset + cut_out_x, y_offset + plot_h, fill=FusionTheme.DANGER.value[1], dash=(4, 4))
        self.charts_canvas.create_text(x1_offset + cut_out_x - 8, y_offset + 15, text="Cut-out", fill=FusionTheme.DANGER.value[1], font=("Arial", 7), angle=90)

        # Plot labels/ticks
        self.charts_canvas.create_text(x1_offset + 15, y_offset + plot_h + 10, text="0", fill=FusionTheme.TEXT_MUTED.value[1], font=("Arial", 8))
        self.charts_canvas.create_text(x1_offset + plot_w - 15, y_offset + plot_h + 10, text="25m/s", fill=FusionTheme.TEXT_MUTED.value[1], font=("Arial", 8))
        ...
        self.charts_canvas.create_rectangle(x2_offset, y_offset, x2_offset + plot_w, y_offset + plot_h, fill="#0F172A", outline=FusionTheme.BORDER.value[1])
        self.charts_canvas.create_text(x2_offset + plot_w // 2, y_offset - 10, text="Turbine Power Curve (kW)", fill=FusionTheme.ACCENT.value[1], font=("Arial", 9, "bold"))
        ...
        if len(p_points) > 1:
            self.charts_canvas.create_line(p_points, fill=FusionTheme.ACCENT.value[1], width=2)

        # Plot labels
        self.charts_canvas.create_text(x2_offset + 15, y_offset + plot_h + 10, text="0", fill=FusionTheme.TEXT_MUTED.value[1], font=("Arial", 8))
        self.charts_canvas.create_text(x2_offset + plot_w - 15, y_offset + plot_h + 10, text="25m/s", fill=FusionTheme.TEXT_MUTED.value[1], font=("Arial", 8))
        self.charts_canvas.create_text(x2_offset + 25, y_offset + 12, text=f"{int(p_rated)}kW", fill=FusionTheme.TEXT_MAIN.value[1], font=("Arial", 8))
    ...
    def redraw_capex_bar(self):
        ...
        w_found = w - (w_turb + w_driv + w_tow)

        # Colors from theme
        c_turb = FusionTheme.INFO.value[1]
        c_driv = FusionTheme.SUCCESS.value[1]
        c_tow = FusionTheme.ACCENT.value[1]
        c_found = "#4B5563"  # Gray
